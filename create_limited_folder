...
dd if=/dev/zero of=$image_of_file bs=1M count=$size_mb
mke2fs -t ext4 -F "$image_of_file" > /dev/null 2>&1 #formating as ext4
mkdir -p $mount_folder #creating a directory for mounting if it doesnt exist
